...
- `CURATOR_SERVER_FETCH` (default `false`): Krystal's Cloudflare challenges datacenter egress, so by default the **browser** fetches Krystal. A GET whose data is not cached answers `{"needFeed": [recipe]}`; the browser runs the recipe's requests (six at a time), posts the payloads to `POST /feed/{kind}`, and asks again. Multi-round datasets (a wallet's vault details, leaderboard pages) come back as `next`. Entries are cached per user, so no user can poison another's screener. Set `true` only where the server's egress is accepted by Krystal.
- `CURATOR_RPC_<chainId>` (optional): chain RPC used by the alert loop for live pool prices (`slot0` / V4 `StateView.getSlot0`), so out-of-range and edge alerts do not depend on Krystal or on the browser being open. Public defaults per chain; empty disables.
