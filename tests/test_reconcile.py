...
def test_reconcile_matches_on_address_and_reports_deltas():
    ...
    other = [
        _pool("0xa", source="rhpools", tvl=110.0, vol=1250.0, fee=9.0),
        _pool("0xb", source="rhpools", tvl=103.0, vol=1000.0, fee=10.0),
        _pool("0xz", source="rhpools", tvl=1.0, vol=1.0, fee=1.0),
    ]
    rec = reconcile(primary, other)
    assert rec.other == "rhpools"
    ...
    assert a.headline == "+10%" and a.describe() == "vs rhpools: tvl +10%  vol24 +25%  fee24 -10%"
    b = rec.by_address["0xb"]
    assert b.level == "ok" and b.worst == pytest.approx(3.0)
    assert rec.flagged == 1
    assert rec.summary() == "vs rhpools: 2 matched, 1 ≥15% off, 1 only here, 1 only there"


def test_unknown_or_zero_tvl_is_not_compared():
    primary = [_pool("0xa", source="krystal", tvl=100.0, vol=1000.0, fee=10.0)]
    unknown = _pool(
        "0xa", source="rhpools", tvl=0.0, vol=1100.0, fee=10.0, unknown=frozenset({"tvl"})
    )
    ...
    zero = _pool("0xa", source="rhpools", tvl=0.0, vol=0.0, fee=0.0)
...
def test_curate_attaches_recon():
    ...
    other = [
        _pool(p.address, source="rhpools", tvl=p.tvl * 1.3, vol=p.s24h.volume, fee=p.s24h.fee)
        for p in pools[:5]
    ]
...
def test_reference_source_picks_the_other_feed(monkeypatch):
    assert api.reference_source(4663, "krystal") == "rhpools"
    assert api.reference_source(4663, "rhpools") == "krystal"
    assert api.reference_source(8453, "krystal") is None  # rhpools only indexes Robinhood
    assert api.fetch_reference_pools(8453, source="krystal") is None
    seen = {}
    monkeypatch.setattr(api, "fetch_pools", lambda c, **kw: seen.update(kw) or [])
    api.fetch_reference_pools(4663, source="rhpools", rhpools_url="http://x")
    assert seen == {"source": "krystal", "rhpools_url": "http://x"}
